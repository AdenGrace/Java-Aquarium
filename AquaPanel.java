...
public class AquaPanel extends JPanel implements ActionListener {
    ...
    private String[] names = {"Add Animal", "Add Plant", "Duplicate Animal", "Sleep", "Wake up", "Reset", "Food", "Info", "Decorator", "Exit"};
    public Set<Swimmable> animals;
    public Set<Immobile> plants;
    ...
    public AquaPanel(AquaFrame f) {
        ...
        subject = new Subject();
        foodObserver = new FoodObserver(f, subject);
    ...
    public void duplicateAnimals() {
        if (animals.size() == 5) {
            JOptionPane.showMessageDialog(frame, "You cannot add more than 5 animals");
        } else {
            DuplicateAnimalDialog dial = new DuplicateAnimalDialog(frame, this, "Duplicate an animal");
            dial.setVisible(true);
        }
    }
    ...
    public void addPlants() {
        if (plants.size() == 5) {
            JOptionPane.showMessageDialog(frame, "You cannot add more than 5 plants");
        } else {
            AddPlantDialog dial = new AddPlantDialog(frame, this, "Add a plant to aquarium");
            dial.setVisible(true);
        }
    }
    ...
    public void addAnimal(String animal, int sz, int hor, int ver, Color c) {
        ...
        Swimmable sw = animalFactory.produceSeaCreature(animal);
        sw.set(subject, sz, 0, 0, hor, ver, c, this);
    ...
    public void duplicateAnimal(Swimmable sw, int sz, int hor, int ver, Color c) {
        Swimmable cloned = sw.clone();
        cloned.set(subject, sz, 0, 0, hor, ver, c, this);
    ...
    public void addPlant(String plant, int sz) {
        PlantFactory plantFactory = new PlantFactory();
        Immobile i = plantFactory.produceSeaCreature(plant);
        // choose random x and y
        Random random = new Random();
        int x = random.nextInt(this.frame.getWidth() - sz * 2) + sz;
        int y = random.nextInt(this.frame.getHeight() - sz * 2) + sz;
        i.set(sz, x, y);
    ...
    public void decorator() {
        DecoratorDialog dial = new DecoratorDialog(frame, this, "JPanelDecorator");
        dial.setVisible(true);
    }
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == b_num[0]) // "Add Animal"
            addAnimals();
        if (e.getSource() == b_num[1]) // "Add Plant"
            addPlants();
        if (e.getSource() == b_num[2]) // "Duplicate Animal"
            duplicateAnimals();
        else if (e.getSource() == b_num[3]) // "Sleep"
            stop();
        else if (e.getSource() == b_num[4]) // "Wake up"
            start();
        else if (e.getSource() == b_num[5]) // "Reset"
            reset();
        else if (e.getSource() == b_num[6]) // "Food"
            food();
        else if (e.getSource() == b_num[7]) // "Info"
            info();
        else if (e.getSource() == b_num[8]) // "Decorator"
            decorator();
